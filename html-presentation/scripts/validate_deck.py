...
10. **Material Symbols** — verifies the correct stylesheet URL and class name
...
27. **Invalid Material Symbols** — fails when a slide contains a
   ``material-symbols-rounded`` span whose text is not a valid icon name.
   When ``scripts/material_symbols_names.txt`` is present (generated by
   ``update_icon_list.py``), performs exhaustive validation against all
   ~4 000 known icon names; otherwise falls back to a small blocklist of
   known-bad names. Run ``update_icon_list.py`` once after install.
...
30. **font-size in px** — fails when a CSS ``font-size`` property inside slide
   content uses ``px`` units; all font sizes (including Material Icon sizes)
   must use ``rem`` for consistent scaling across browsers and zoom levels
31. **Code-block leading newline** — warns when a ``<div class="code-block">``
   opening tag is immediately followed by a newline before any content; with
...
_NAMES_FILE = Path(__file__).parent / "material_symbols_names.txt"
VALID_SYMBOL_NAMES: frozenset[str] = (
    frozenset(_NAMES_FILE.read_text(encoding="utf-8").split())
...
ICON_BLOCKLIST: dict[str, str] = {
    "settings_off":  "toggle_off, layers_clear, or block (does not exist in Material Symbols Rounded)",
...
LI_LEADING_SYMBOL_RE = re.compile(
    ...
    + SYMBOL_CHAR_CLASS
    + r"|&#x[0-9A-Fa-f]+;"
    r"|<span[^>]*class=\"material-symbols-rounded\"[^>]*>.*?</span>"
...
def validate(html_path: Path) -> tuple[list[str], list[str], list[str]]:
    ...
    if has_icons:
        if MATERIAL_URL_RE.search(html):
            passes.append("Material Symbols: correct URL and class")
        else:
            fails.append("Material Symbols class used but stylesheet URL is wrong or missing")
    else:
        passes.append("No Material Symbols used (nothing to check)")
    ...
    # 27. Invalid Material Symbols — exhaustive check when names file present, else blocklist
    invalid_icon_hits: list[str] = []
    for sb in slide_blocks:
        sid = sb.group(2)
        ...
        for im in MATERIAL_ICON_SPAN_RE.finditer(slide_body):
            ...
            if VALID_SYMBOL_NAMES:
                if icon_name not in VALID_SYMBOL_NAMES:
                    invalid_icon_hits.append(
                        f"{sid} line {ln}: '{icon_name}' is not a valid Material Symbol "
                        f"— check spelling or run update_icon_list.py to refresh the names list"
                    )
            elif icon_name in ICON_BLOCKLIST:
                alt = ICON_BLOCKLIST[icon_name]
                invalid_icon_hits.append(
                    f"{sid} line {ln}: '{icon_name}' is blacklisted — use {alt} instead"
                )

    if invalid_icon_hits:
        fails.append(
            f"{len(invalid_icon_hits)} invalid Material Symbol(s) found: "
            f"{invalid_icon_hits[0]}"
            + (f"  (+{len(invalid_icon_hits) - 1} more)" if len(invalid_icon_hits) > 1 else "")
        )
    else:
        mode = f"({len(VALID_SYMBOL_NAMES)} known names)" if VALID_SYMBOL_NAMES else "(blocklist only — run update_icon_list.py for full validation)"
        passes.append(f"All Material Symbol names valid {mode}")

    # 30. font-size in px — all font-size values must use rem
    fs_px_hits: list[str] = []
    for sb in slide_blocks:
        sid = sb.group(2)
        slide_body = sb.group(3)
        for m in FONT_SIZE_PX_RE.finditer(slide_body):
            pos = sb.start() + len(sb.group(1)) + m.start()
            ln = line_no(html, pos)
            fs_px_hits.append(f"{sid} line {ln}: '{m.group(0)}' — use rem (e.g. 1rem, 1.5rem)")
    if fs_px_hits:
        fails.append(
            f"{len(fs_px_hits)} font-size px value(s) found (must use rem): "
            f"{fs_px_hits[0]}"
            + (f"  (+{len(fs_px_hits) - 1} more)" if len(fs_px_hits) > 1 else "")
        )
    else:
        passes.append("No font-size px values found — all use rem")

    # 31. Code-block leading newline — renders as blank line due to white-space: pre-wrap
    cb_leading_ws_hits: list[str] = []
    for sb in slide_blocks:
        sid = sb.group(2)
        slide_body = sb.group(3)
        for m in CB_LEADING_WS_RE.finditer(slide_body):
            pos = sb.start() + len(sb.group(1)) + m.start()
            ln = line_no(html, pos)
            cb_leading_ws_hits.append(
                f"{sid} line {ln}: code-block opening tag followed by newline — "
                f"first <span> must be on same line as opening tag (white-space: pre-wrap)"
            )
    if cb_leading_ws_hits:
        warns.append(
            f"{len(cb_leading_ws_hits)} code-block(s) with leading newline (will render blank line): "
            f"{cb_leading_ws_hits[0]}"
            + (f"  (+{len(cb_leading_ws_hits) - 1} more)" if len(cb_leading_ws_hits) > 1 else "")
        )
    else:
