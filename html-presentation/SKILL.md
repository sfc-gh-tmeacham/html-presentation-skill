...
16. **SVG layout integrity — five hard rules:**
    ...
    - **Always set `overflow="visible"` on every `<svg>` element.** SVGs default to `overflow: hidden`, which hard-clips anything that extends even 1px beyond the viewBox boundary. Setting `overflow="visible"` prevents bottom (and edge) clipping without requiring a perfectly tight viewBox.
    - **Add 20px bottom padding to every viewBox.** The viewBox height MUST be at least 20px greater than `y + height` of the lowest element in the diagram. Never set the viewBox height equal to the last element's bottom edge — even a 1px overrun clips silently.
