...
7. **Use full viewport efficiently.** Set `max-width: min(1200px, 92vw)`, use `clamp()`-based padding so content fills available space. Projectors and wide monitors have room — don't waste it with oversized margins. Keep breathing room *between* elements; goal is balanced density, not clutter.
